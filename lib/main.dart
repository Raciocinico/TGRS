...
// Ver si el usuario ya vio el tutorial
...
// tap on icon chat to the next screen, empty screen to explore(buscar en IG), FEED(Publicaciones amigos) Chats(contacts)// (navigation and app bar)
  @override
  Widget build(BuildContext context) {
    return ShowCaseWidget(
      onComplete: (index, key) {
        if (!mounted) return;
        setState(() {
          _showFab = false; // 👈 AQUÍ desaparece
        });
      },
      builder: Builder(
...
// 🔹 MENÚ LATERAL IZQUIERDO
                  ...
                  // 🎮 2️⃣ FAB REAL (interacción)
                  if (_showFab && !_isMenuOpen && !_isListOpen)
                    Positioned(
                      top: 10,
                      left: 10,
                      child: Column(
      ...
      body: SafeArea(
        child: Row(
          children: [
            Expanded(
              flex: 3,
              child: _LeftSettingsPanel(),
            ),
            Expanded(
              flex: 6,
              child: _RightProfilePanel(
                onRestartTutorial: () {
...
class _LeftSettingsPanel extends StatelessWidget {
  const _LeftSettingsPanel();
...
class _RightProfilePanel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const String name = 'Edward Kenway';
    const String username = '@dumbcorsair';
          ...
          child: Column(
            ...
            children: [
              ...
              _SettingsItem(
                ...
                subtitle: 'Verification, Customization, Privacy',
                onTap: () {},
              ),
              _SettingsItem(
                icon: Icons.groups_2_outlined,
                title: 'Social Settings',
                subtitle: 'Saved, Groups, Follows',
                onTap: () {},
              ),
              _SettingsItem(
                icon: Icons.notifications_none,
                title: 'Notifications Settings',
                subtitle: 'Messages, Calls, Comments',
                onTap: () {},
              ),
              const Spacer(),
              _SettingsItem(
                ...
                subtitle: 'Vuelve a ver la introducción',
                onTap: () async {
                  final prefs = await SharedPreferences.getInstance();
                  await prefs.setBool('hasSeenTutorial', false);
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        ...
                        fontStyle: item.type == FeedType.notification
                            ? FontStyle.italic
                            : FontStyle.normal,
                      ),
                    ),
